    ...
    <div>
      ...
      <div class="space-y-3">
        ...
        <%= if Enum.empty?(@sessions) do %>
          <div class="text-center py-12 text-base-content/50">
            No sessions yet. Send requests with an <code class="bg-base-200 px-1 rounded">X-Session-Id</code> header to create one.
          </div>
        <% end %>
      </div>
      ...
      <%= if @page > 1 do %>
        <div class="flex justify-center mt-6 gap-2">
          <a href={~p"/routers/#{@router.id}/sessions?page=#{@page - 1}"} class="btn btn-sm btn-ghost">← Prev</a>
          <span class="btn btn-sm btn-disabled">Page <%= @page %></span>
          <a href={~p"/routers/#{@router.id}/sessions?page=#{@page + 1}"} class="btn btn-sm btn-ghost">Next →</a>
        </div>
      <% end %>
    </div>
    """
