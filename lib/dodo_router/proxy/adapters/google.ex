defmodule DodoRouter.Proxy.Adapters.Google do
  ...
  def call(request, %RoutingStep{} = step, api_key) do
    url = "https://generativelanguage.googleapis.com/v1beta/models/#{step.model}:generateContent?key=#{api_key}"
  ...
  def stream(request, %RoutingStep{} = step, api_key, send_chunk) do
    url = "https://generativelanguage.googleapis.com/v1beta/models/#{step.model}:streamGenerateContent?key=#{api_key}&alt=sse"
    body = build_gemini_request(request)
  ...
  defp build_gemini_request(request) do
    ...
    body = if system_instruction, do: Map.put(body, "systemInstruction", %{"parts" => [%{"text" => system_instruction}]}), else: body

    # Generation config
    gen_config = %{}
    gen_config = if request["temperature"], do: Map.put(gen_config, "temperature", request["temperature"]), else: gen_config
    gen_config = if request["top_p"], do: Map.put(gen_config, "topP", request["top_p"]), else: gen_config
    gen_config = if request["max_tokens"], do: Map.put(gen_config, "maxOutputTokens", request["max_tokens"]), else: gen_config
    gen_config = if request["stop"], do: Map.put(gen_config, "stopSequences", List.wrap(request["stop"])), else: gen_config
  ...
  defp extract_system_and_contents(messages) do
    ...
    contents = Enum.map(other, fn msg ->
  ...
  defp convert_to_openai_format(gemini_response) do
    ...
    finish_reason = case candidate["finishReason"] do
      "STOP" -> "stop"
      "MAX_TOKENS" -> "length"
    ...
    usage = if gemini_response["usageMetadata"] do
      %{
  ...
  defp process_gemini_chunks(acc, chunks) do
    Enum.reduce(chunks, {acc, []}, fn chunk, {acc, openai_chunks} ->
      text = get_in(chunk, ["candidates", Access.at(0), "content", "parts", Access.at(0), "text"]) || ""
      ...
      if text != "" do
        openai_chunk = "data: #{Jason.encode!(%{"choices" => [%{"index" => 0, "delta" => %{"content" => text}}]})}\n\n"
