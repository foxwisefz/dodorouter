defmodule DodoRouter.Logs do
  ...
  @slug_alphabet ~c"abcdefghjkmnpqrstuvwxyz23456789"

  @doc """
  Publishes a log under a public slug. Snapshots a redacted copy of the
  request and response bodies into `public_request_body` / `public_response_body`
  so a later change to redaction rules cannot retroactively expose data.
  """
  def publish_log(%User{} = user, log_id, attrs \\ %{}) do
    case get_log(user, log_id) do
      nil ->
        {:error, :not_found}

      log ->
        slug = log.public_slug || generate_slug()
        ...
        publish_attrs = %{
          ...
          public_title: title,
          public_request_body: redact_body_string(log.request_body),
          public_response_body: redact_body_string(log.response_body)
        }

        log
        |> RequestLog.publish_changeset(publish_attrs)
        |> Repo.update()
    end
  end

  @doc """
  Reverts a published log back to private. Clears all `public_*` fields.
  """
  def unpublish_log(%User{} = user, log_id) do
    case get_log(user, log_id) do
      nil ->
        {:error, :not_found}
      ...
      log ->
        log
        |> RequestLog.unpublish_changeset()
        |> Repo.update()
    end
  end

  @doc """
  Fetches a published log by its public slug. No user argument — this is the
  anonymous read path. Returns nil if the slug is unknown or the log was
  unpublished.
  """
  def get_public_log_by_slug(slug) when is_binary(slug) do
    from(l in RequestLog,
      where: l.public_slug == ^slug and not is_nil(l.published_at),
      preload: [router: [:user]]
    )
    |> Repo.one()
  end
  ...
  @doc """
  Lists all published logs in a session, ordered by insertion time. Used by
  the public session-tree view; only published logs are included even if the
  session contains other private ones.
  """
  def list_public_logs_for_session(session_id) when is_binary(session_id) do
    from(l in RequestLog,
      where: l.session_id == ^session_id and not is_nil(l.published_at),
  ...
  @doc """
  Lists published logs for a given username, newest first. For the public profile.
  """
  def list_public_logs_for_username(username, opts \\ []) when is_binary(username) do
    limit = Keyword.get(opts, :limit, 50)

    from(l in RequestLog,
      join: r in Router,
      on: l.router_id == r.id,
      join: u in DodoRouter.Accounts.User,
      on: r.user_id == u.id,
      where: u.username == ^username and not is_nil(l.published_at),
  ...
  defp generate_slug do
    for _ <- 1..10, into: "", do: <<Enum.random(@slug_alphabet)>>
  end

  defp redact_body_string(nil), do: nil

  defp redact_body_string(body) when is_binary(body) do
    case Jason.decode(body) do
      ...
      _ ->
        DodoRouter.Redact.redact_secrets(body)
    end
  end

  defp redact_body_value(value) when is_map(value) do
    Map.new(value, fn {k, v} -> {k, redact_body_value(v)} end)
  end

  defp redact_body_value(value) when is_list(value) do
    Enum.map(value, &redact_body_value/1)
  end

  defp redact_body_value(value) when is_binary(value),
    do: DodoRouter.Redact.redact_secrets(value)

  defp redact_body_value(value), do: value
