...
**The same rule applies inside `messages`, at the content-block level.** The passthrough above only covers top-level request keys; blocks nested in a message's `content` are consumed by the converter, so each block type needs an IR representation or it is dropped in silence — `document` blocks (base64 PDFs) spent their first months exactly there. The contract: a base64 `document` block becomes an OpenAI `file` part (`file_data` data URI, `title` ↔ `filename`, `cache_control` preserved), and `Adapters.Anthropic.normalize_content_blocks/1` reverses it 1:1, so an Anthropic-to-Anthropic round trip is byte-identical. Document sources OpenAI has no shape for (url, text, content, Files API) ride the IR verbatim: Anthropic-format steps pass them through, and an OpenAI-family step answers with its own 400 rather than a silent drop. `Adapter.all_text_content?/1` must treat any such block as non-text, or `sanitize_messages` JSON-flattens it into prose. Test the full `to_openai_params → build_anthropic_request` seam, not each function alone (`anthropic_format_test.exs`, "document passthrough").
