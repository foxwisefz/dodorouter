...
Comparison uses the latest earlier-starting successful request already recorded in the same router and session, even when requests finish out of order. The explanation includes how many previous messages match, whether tools and the system prompt stayed the same, whether the provider key changed, and the time since the previous attempt completed. Model/provider/key/endpoint changes make that baseline incompatible. An observed change can explain lost reuse, but does not prove which tokens the provider could cache. A uniform explicit requested TTL and overlapping request times support **likely expiry** and **possible race** explanations only when the baseline reported cache reads or writes; provider shard and actual expiry are unavailable. Missing usage is shown as unreported, not a zero-read miss. Old logs have no signatures and cannot be backfilled without the original full outbound payload.
