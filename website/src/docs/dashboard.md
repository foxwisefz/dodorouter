...
Every request, live-streamed in as it happens. Filter by router or favorites. Each log opens into full detail: conversation view, raw request/response JSON, per-attempt fallback trace, timing, cost, and a **Replay** action that reruns the same conversation against a different model and diffs the two results (inline diff, side-by-side, or raw JSON). When a provider dies mid-response and the next one continues the same stream, the failed attempt is badged **mid-stream handoff** and shows the exact partial text the client had already received; while the request is still in flight, the pending row in the list switches live to the backup step serving it.
...
New request logs also show **Cache evidence**: for example, “User message 12 changed,” “Tools changed at item 3,” or “Shared prefix unchanged; provider reported zero cache reads.” The session cache warning includes the same explanation for its first diverging turn. These observations use keyed structural signatures of the final provider payload, computed before truncation, so they remain useful even when request bodies are unavailable. Appending conversation messages is normal growth, not a prefix change.

Comparison uses the latest earlier-starting successful request already recorded in the same router, session, and optional branch, even when requests finish out of order. The explanation includes how many previous messages match, whether tools and the system prompt stayed the same, whether the provider key changed, and the time since the previous attempt completed. Model/provider/key/endpoint changes make that baseline incompatible. An observed change can explain lost reuse, but does not prove which tokens the provider could cache. A uniform explicit requested TTL and overlapping request times support **likely expiry** and **possible race** explanations only when the baseline reported cache reads or writes; provider shard and actual expiry are unavailable. Missing usage is shown as unreported, not a zero-read miss. Old logs have no signatures and cannot be backfilled without the original full outbound payload.
